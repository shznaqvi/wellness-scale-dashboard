...
//        $file = "E:/PortalFiles/JSONS/TPVICS/" . $table . '-' . $file_dt . '.json';
...
//var_dump($value);die(); 
            if ($value != null) {
                ...
                //$create_sql .= " CONSTRAINT [DF_" . $table . "_col_dt]  DEFAULT (getdate()) FOR [col_dt] ";


                //echo $create_sql; die();
                //$create_sql .= " IF NOT EXISTS (SELECT * FROM dbo.sysobjects WHERE id = OBJECT_ID(N'[DF_" . $table . "_col_dt]') AND type = 'D')  ";
                ...
                $json[] = array("error" => 1, "message" => 'CData is Null [] OR Key missing!', "status" => 0, "values" => $value);
